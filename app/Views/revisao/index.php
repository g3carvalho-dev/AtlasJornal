        ...
        <div class="revisao-layout">
            <div class="revisao-lista">
                <div class="revisao-stats-bar">
                    <span class="rev-stat pendente"><i class="fa-regular fa-clock"></i> <?= count($noticias) ;?> pendentes</span>
                </div>
                ...
                <?php else: ?>
                    <table class="revisao-tabela">
                        ...
                        <tbody>
                            ...
                            <tr class="<?= ($noticiaSelecionada && $noticiaSelecionada->getId() == $row['id']) ? 'selecionada' : '' ;?>" onclick="window.location='<?= url('/revisao?noticia=' . $row['id']) ;?>'">
            ...
            <div class="revisao-preview">
                <?php if ($noticiaSelecionada): ?>
                    ...
                    <p class="preview-meta">
                        <i class="fa-regular fa-user"></i> <?= $autorNoticia ? e($autorNoticia->getNome()) : 'Desconhecido' ;?>
                        &nbsp;&bull;&nbsp;
                        <i class="fa-regular fa-calendar"></i> <?= format_date($noticiaSelecionada->getDataCriacao(), 'd/m/Y \à\s H:i') ;?>
                    </p>
                    ...
                    <form class="revisao-form" method="POST" action="">
                        <div class="revisao-observacao">
                            ...
                            <textarea name="observacao" rows="3" placeholder="Deixe um comentário, solicite ajustes ou registre sua observação..."></textarea>
                            <small>Seu comentário será visível para o autor do artigo.</small>
                        </div>

                        <div class="revisao-botoes">
                            <a href="<?= url('/noticia/' . $noticiaSelecionada->getId() . '/editar') ;?>" class="btn-rev btn-editar">
                                <i class="fa-solid fa-pen"></i> EDITAR ANTES DE APROVAR
                            </a>
                            <button type="submit" formaction="<?= url('/revisao/arquivar/' . $noticiaSelecionada->getId()) ;?>" class="btn-rev btn-arquivar">
                                <i class="fa-solid fa-box-archive"></i> ARQUIVAR
                            </button>
                            <button type="submit" formaction="<?= url('/revisao/rejeitar/' . $noticiaSelecionada->getId()) ;?>" class="btn-rev btn-rejeitar">
                                <i class="fa-solid fa-xmark"></i> REJEITAR
                            </button>
                            <button type="submit" formaction="<?= url('/revisao/aprovar/' . $noticiaSelecionada->getId()) ;?>" class="btn-rev btn-aprovar">
